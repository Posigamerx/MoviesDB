...
const TopRated = () => {
  ...
  useEffect(() => {
    const fetchTopRated = async (page, type) => {
      setLoading(true);
      try {
        const movieEndpoint = type === "1" ? 'movie' : 'tv';
        const topRatedResponse = await axios.get(
          `https://api.themoviedb.org/3/${movieEndpoint}/top_rated`,
          {
            params: {
        ...
        if (type === "1") {
          setMovies(topRatedResponse.data.results);
        } else {
          setTvSeries(topRatedResponse.data.results);
        }
  ...
  return (
    <div className='bg-gray-900 bg-no-repeat bg-fixed bg-center'>
      <div>
      <h1 className='pt-20 mb-2 ml-5 text-2xl sm:pt-28 sm:mb-5 sm:ml-10 sm:text-4xl font-bold text-white'>Top Rated  </h1>
